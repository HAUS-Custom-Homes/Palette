...
**Start here next session (set 2026-09-24 on Trevor_Lenovo):**
1. **TikTok videos** (Trevor's pick for the next session; see "NEXT SESSION" at the top). Ask for the links that
   failed and read `/install/attempts` first.
2. **Real-phone check of 9/24's share flow** on Trevor's Fold and iPhone: share a post, the save page (haus, lookbook
   or a new one, note), Save with its progress card, "Saved to Palette / Done", the Done at the end of the details,
   and the header's "+ New". Also the phone header (notch, pinned, no zoom), which Chromium could not fully prove.
3. **Send the team to** Settings (gear), Get the app and share sheet. The iPhone Shortcut is one tap now.
4. Trevor: load the rebuilt extension (`extension/.output/chrome-mv3`) and try **Save the video** on a Reel.
5. Trevor: press Deploy on the Cloudflare "Redirect from WWW to root" rule (`www.hauspalette.com` still 404s).
6. Before any screen change ships: `npm run dev:agents`, `tools/qc-layout.js`, a screenshot (CLAUDE.md).
...
- Tags are applied automatically (Trevor, 9/22), and doubtful ones are listed under Needs me, "Tags to check".
  Labelling the golden set and `npm run eval` would measure the model; worth it only if the tags feel unreliable.
